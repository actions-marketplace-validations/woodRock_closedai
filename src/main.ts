...
async function run() {
  ...
  const repoRoot = process.env.WORKSPACE_DIR
  ...
  if (statusDoc.exists) {
    ...
    if (now - lastSeen < 90000) {
      logInstruction(
        0,
        'INFO',
        `Another bot instance is active (last seen ${Math.round((now - lastSeen) / 1000)}s ago). Skipping...`,
      )
      cleanup()
      return
    }
  ...
  if (isPolling) {
    ...
    bot.on('message', async (ctx) => {
      ...
      if (text || media) {
        await processOneMessage(text, ctx.chat.id, repoRoot, undefined, media, true).catch(
          console.error,
        )
      }
    })
  ...
  try {
    ...
    while (initialQueueRetryCount < 10) {
      // Limit initial retries to prevent infinite loops if API is down
    ...
    const finalQueueCheck = await db
      .collection('queue')
